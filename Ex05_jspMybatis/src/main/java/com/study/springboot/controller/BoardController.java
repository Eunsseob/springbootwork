...
public class BoardController {
	...
	public String listPage(Model model) {
		...
		// int result = boardService.totalRecord();
	 ...
	 * 			String bno = Integer.parseInt(request.getParameter("boardno"));
	 * 		
	 * 	2. @ReqeustParam 어노테이션 사용하는 방법
	 * 		: 메서드 위에 에노테이션을 넣는방법
	 * 		ex)
	 * 		@RequestMapping("detail")
	 * 		public String view(@RequestParam(value="boardno") int bno,
	 * 												@RequestParam(value="writer") String name), String name,
	 ...
	 *	
	 *			스프링컨테이너가 해당 객체를 기본생성자로 생성 후 setter메소드를 찾아(반드시 빈 생성자 있어)
	 *			요청시 전달값을 해당 필드에 담아주는 내부적인 원리
	 *
	 *			** 반드시 name 속성값(키값)과 담고자하는 필드명이 동일해야 함
	 *
	 *			ex) 
	 ...
	 * 	4. 커맨드 객체 방식
	 *			: 객체를 받을 때 사용
	 *				요청시 전달한 값의 키값(name값)을 bean클래스에 담고자하는 필드명으로 작성	 
	 */
	@RequestMapping("/detail")
	...
	@PostMapping("/write")
	public String write(@ModelAttribute Board b, Model model) {
